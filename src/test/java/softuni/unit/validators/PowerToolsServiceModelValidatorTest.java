...
import softuni.project.services.validations.PowerToolsServiceModelValidatorImpl;
...
class PowerToolsServiceModelValidatorTest {
    PowerToolsServiceModel model;
    @Mock
    private PowerToolsServiceModelValidator validator;

    @BeforeEach
    public void setUp() {
        validator = new PowerToolsServiceModelValidatorImpl();
        ...
        model.setToolCondition("Brand New");
    ...
    @Test
    void isValid_WhenToolConditionIsNull_ShouldFalse(){
        model.setToolCondition(null);
        boolean result = validator.isValid(model);
        assertFalse(result);
    }
