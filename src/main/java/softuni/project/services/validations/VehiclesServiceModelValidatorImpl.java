...
@Service
public class VehiclesServiceModelValidatorImpl implements VehiclesServiceModelValidator {
    @Override
    public boolean isValid(VehicleServiceModel model) {
        return isNameValid(model.getName()) && isDescriptionValid(model.getDescription())
                ...
                && isBrandValid(model.getBrand()) && isModelValid(model.getModel());
    }
    ...
    boolean isBrandValid(String brand) {
    return brand != null;
    }
